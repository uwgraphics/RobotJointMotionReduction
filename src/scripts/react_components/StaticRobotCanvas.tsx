...
export interface quaternion_canvas_props {
    ...
    staticRobotScene: StaticRobotScene, // The scene that the BuildPage should show
    setStaticRobotSceneOptionPanelActive: () => void,
}
...
export class StaticRobotCanvas extends Component<quaternion_canvas_props, quaternion_canvas_state> {
    ...
    constructor(props:quaternion_canvas_props) {
        ...
        this.initScene(this.props.staticRobotScene);
    }
     ...
     */
    addPointer(rScene:StaticRobotScene, pos:T.Vector3) {
     ...
     */
    removePointer(rScene:StaticRobotScene) {
        const scene = rScene.scene();
        if (scene.children.indexOf(this._pointer) !== -1) {
     ...
     */
    initScene(quaternionSpaceScene:StaticRobotScene) {
     ...
     */
    mount(staticRobotScene:StaticRobotScene, canvas?:HTMLCanvasElement | null) {
        if (canvas) {
            this.initScene(staticRobotScene);
            ...
            controls.update = (() => {
                ...
                staticRobotScene.render(); // Tell RobotScene that it needs to rerender because the camera's position changed
                return result;
            });
            controls.target.y = 0;
            ...
            staticRobotScene.addView(this._orbitView);
            staticRobotScene.render(); // Make sure that the new SceneView is at least rendered to once
     ...
     */
    unmount(staticRobotScene:StaticRobotScene, canvas?:HTMLCanvasElement | null) {
        ...
        if (this._orbitView) {
            staticRobotScene.removeView(this._orbitView);
        ...
        this.removePointer(staticRobotScene); // removes the pointer if it is in the given Scene
    }
    ...
    componentDidUpdate(prevProps:quaternion_canvas_props) {
        if (prevProps.staticRobotScene !== this.props.staticRobotScene) {
            ...
            this.unmount(prevProps.staticRobotScene, this._canvasRef.current);
            this.mount(this.props.staticRobotScene, this._canvasRef.current);
        }
    }
    ...
    componentWillUnmount() {
        this.unmount(this.props.staticRobotScene, this._canvasRef.current);
    }
    ...
    dropHandler(event: any) {
        ...
        this.props.staticRobotScene.addChildRobot(robot, 5);
        this.props.staticRobotScene.render();
    }
